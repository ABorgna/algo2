...
\tadOperacion{quitarRegistros}{conj(registro)/rs, campo/c, valor/v}{conj(registro)}
    {mismosCampos($rs$) $\land$ $c$ $\in$ todosCampos($rs$) $\land$ (cEsString?($c$) $\iff$ vEsString?($v$))}
...
\tadOperacion{\cuidado rContieneNombre?}{registro/r, string/nombre}{bool}{}

\tadOperacion{\cuidado recortaR}{registro/r, conj(campo)/cs}{registro}{cs $\subseteq$ rCampos(r) }
\tadOperacion{\cuidado recortaCR}{conj(registro)/rs, conj(campo)/cs}{conj(registro)}{mismosCampos(rs) $\land$ cs $\subseteq$ todosCampos(rs)}
\tadOperacion{\cuidado mismosCampos}{conj(registro)/rs}{bool}{}
\tadOperacion{\cuidado todosCampos}{conj(registro)/rs}{conj(campo)}{}
