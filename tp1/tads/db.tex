...
\tadOperacion{existeTablaConModificaciones}{database/db, nat/n, conj(nombreTabla)/nts}{bool}{
    $nts \subseteq$ nombresTablas(db)
}
...
{
  verJoinAux(dbTabla(insertar(db, nt, r), $Pi_1$(obtener(dbJoins(db), nj))), dbTabla(insertar(db, nt, r), $Pi_2$(obtener(dbJoins(db), nj))), dameUno(tClaves(t) $\cap$ tClaves(t')))
}

\tadAxioma{dbVerJoin(borrar(db, nt, c, v), nj)}{
  verJoinAux(dbTabla(borrar(db, nt, c, v), $Pi_1$(obtener(dbJoins(db), nj))), dbTabla(borrar(db, nt, c, v), $Pi_2$(obtener(dbJoins(db), nj))), dameUno(tClaves(t) $\cap$ tClaves(t')))
}
...
\tadAxioma{existeTablaConModificaciones(db,tModificaciones(dbTabla(masModificada(db))),nombresTablas(db))}{true}

\tadAxioma{existeTablaConModificaciones(db,n,nts)}{
    \IF $\emptyset$?(nts) THEN
        false
    ELSE
        tModificaciones(dbTabla(dameUno(nts))) = n $\lor$ \\
        existeTablaConModificaciones(db,n,sinUno(nts))
    FI
}
...
\tadAxioma{joinRegistros(t, tRegistros, t', campo, sumaRegistros)}{
  \\
  \IF $\varnothing$?(tRegistros) THEN 
    sumaRegistros
  ELSE
    {\IF $\neg$ rVacio?(dameRegistro(t', campo, rValor(dameUno(tRegistros), campo))) THEN
      joinRegistros(t, sinUno(tRegistros), t', Ag(unionRegistros(dameUno(tRegistros), dameRegistro(t', campo, rValor(dameUno(registros), campo))), sumaRegistros)
    ELSE
      joinRegistros(t, sinUno(tRegistros), t', sumaRegistros)
    FI}
  FI}
