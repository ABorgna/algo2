...
\tadOperacion{crearTrigger}{database/db, nombreTrigger/ntg, trigger/tg}{database}
    ...
    $\land$ tCampos(dbTabla(db, $\Pi_2$(tg))) $\subseteq$ rCampos($\Pi_3$(tg)))}
...
\tadOperacion{masModificadaAux}{database, conj(tombreTabla), tabla}{nombreTabla}{}
...
\tadAxioma{masModificada(db)}{masModificadaAux(db, sinUno(nombresTablas(db)), dameUno(nombresTablas(db)))}
\tadAxioma{masModificadaAux(db, nts, tMasModificada)}{
\IF $\varnothing$?(nts) THEN 
    tMasModificada 
ELSE
    {\IF tModificaciones(dbTabla(db, dameUno(nts))) $\geq$ tModificaciones(dbTabla(db, tMasModificada)) THEN
        masModificadaAux(db, sinUno(nts), dameUno(nts)) 
    ELSE
        masModificadaAux(db, sinUno(nts), tMasModificada) 
    FI}
FI}
