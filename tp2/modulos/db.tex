...
\algoritmo{iHayTabla}{
    ...
    \State $res \gets definido?(db.tablasTree,nt)$                  \comment \bigo(1)
}
...
\algoritmo{iTabla}{
    ...
    \State $res \gets obtener(db.tablasTree,nt)$                    \comment \bigo(1)
}
...
\algoritmo{iTablaMaxima}{
    ...
    \State $res \gets *db.tablaMaxima$                              \comment \bigo(1)
}
...
\algoritmo{iBorrarJoin}{
    ...
    \State $borrar(obtener(db.vistasJoin,nt_1),nt_2)$               \comment \bigo(1)
    \If{$!haySiguiente(crearIt(obtener(db.vistasJoin,nt_1)))$}      \comment \bigo(1)
        \State $borrar(db.vistasJoin,nt_1)$                         \comment \bigo(1)
    \EndIf
}
...
\algoritmo{iVistaJoin}{
    ...
    \param{in}{$nt_1$}{$nombreTabla$},
    \param{in}{$nt_2$}{$nombreTabla$}
    }{itConj(registro)}{\bigo(in)}{
    \var $v : vistaJoin$
    \var $it : itLista(operacionJoin)$
    \var $t_1, t_2 : tabla$
    \State $v \gets obtener(obtener(db.vistasJoin,nt_1),nt_2)$      \comment \bigo(1)
    \State $t_1 \gets tabla(db,nt_1)$                               \comment \bigo(1)
    \State $t_2 \gets tabla(db,nt_2)$                               \comment \bigo(1)
    ...
    \While{$haySiguiente(it)$}                                      \comment \bigo(1)
        ...
        \Else
            \If{$v.tipo$}                                           \comment \bigo(1)
                \If{$definido?(v.joinsNat, valorNat(d))$}           \comment \bigo(1)
                ...
                \EndIf
            \Else
                \If{$definido?(v.joinsString, valorString(d))$}     \comment \bigo(1)
...
\algoritmo{iCombinarRegistros}{
    ...
    \While{$haySiguiente(it)$}                                      \comment \bigo(1)
        \State \icomment El loop se ejecuta |campos(r)| veces, acotado por constante
        \State $definir(res, siguiente(it).clave, siguiente(it).significado)$   \comment \bigo(1)
    \EndWhile
