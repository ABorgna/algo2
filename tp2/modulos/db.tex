...
{   \param{in/out}{$db$}{$base$},
    ...
    \param{in}{$d$}{$dato$}}
{}
{$db \igobs db_0 \land nt \in tablas(db) \land_L tipo?(d) \igobs tipoCampo(c,dameTabla(nt,db))$}
...
{$db \igobs generarVistaJoin(nt_1,nt_2,c,db_0)$}
{$in$}
{}
...
\donde{vistaJoin}{tupla($campo$: campo, $buffer$: buffer, $joins$: joins)}
\donde{joins}{diccLog($dato$, $registro$)}
...
\begin{Rep}{$estr$}{$e$}
    ...
    \repfunc{No hay inserciones de indices ya cacheados en un buffer (sin borrar antes)}
        ...
        \hspace*{10em} d \in claves(v.joins) \; \land_L obtener(v.campo, op.reg) \igobs d ) \implies \\
        \hspace*{8em} (\exists op' : operacionJoin) \; (op' \in v.buffer \; \land_L \neg op'.esInsercion \; \land \\
    ...
    \repfunc{Todos los joins posibles están cacheados en vistasJoins y no hay modificaciones en el buffer, o la inserción está en el buffer y no se la borra luego}
        ...
        \hspace*{8em} (def?(obtener(v.campo,r), v.joins) \land_L \\
        \hspace*{10em} r \igobs obtener(obtener(v.campo,r), v.joins) \land \\
    ...
    \repfunc{vistasJoins tiene joins válidos o joins que seran eliminados}
        ...
        \hspace*{6em} t_a \igobs obtener(nt, e.tablasTree) \land t_b \igobs obtener(nt', e.tablasTree) \land \\
        \\
        \hspace*{6em} (\forall r : registro) \; (def?(obtener(v.campo,r), v.joinsNat) \land_L \\
        \hspace*{8em} r \igobs obtener(obtener(v.campo,r), v.joinsNat)) \implies_L (\\
