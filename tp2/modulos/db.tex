...
\algoritmo{iGenerarVistaJoin}{
    ...
    \State \icomment (Por el pre, c es siempre clave)
    \If{$esIndice(t_1, c) \algAnd esIndice(t_2, c)$}                    \comment \bigo(1)
        ...
        \EndIf
    \ElsIf{$esIndice(t_1, c)$}                                          \comment \bigo(1)
        \State $recorreTabla1 \gets true$                               \comment \bigo(1)
        \State $t_a \gets t_1$                                          \comment \bigo(1)
        \State $t_b \gets t_2$                                          \comment \bigo(1)
    \Else
...
\algoritmo{iVistaJoin}{
    ...
    \While{$haySiguiente(it)$}                                      \comment \bigo(1)
        ...
        \If{$op.esInsercion$}
            \var $rs : conj(registro)$
            \var $r, r_b : registro$
            \State \icomment $c$ siempre es clave (por el pre),
                             por lo que buscar tendrá complejidad \bigo($L + log(n)$) si $c$ es índice
            \If{$!op.enTablaB$}                                     \comment \bigo(1)
                \State $rs \gets buscar(t_2, definir(nuevoDiccLog(),v.campo,d))$
                                        \comment \bigo($L + log(m)$) si es índice, \bigo($L * m$) sino
            \Else
                \State $rs \gets buscar(t_1, definir(nuevoDiccLog(),v.campo,d))$
                                        \comment \bigo($L + log(n)$) si es índice, \bigo($L * n$) sino
            \EndIf
            \If{$!esVacio?(rs)$}                                    \comment \bigo(1)
                \State $r_b \gets siguiente(crearIt(rs))$           \comment \bigo(1)
