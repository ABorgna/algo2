...
\begin{ABS}{e}{estrDicc}{d}{dicc(string,$\alpha$)}
     ...
     \absfunc{}
    {
        (\forall s : string\ def?(s, d) \Leftrightarrow existe(s, 0, e.raiz)\ \land_L
    }
...
{bool}
{}
\tadAxioma{existe(s, k, n)}{\IF (*n).hijos[ord(s[k])] == NULL THEN false ELSE {\IF k==long(s)-1 $\land$ (*n).hijos[ord(s[k])] != NULL THEN true ELSE existe(s,k+1, (*n).hijos[ord(s[k])] ) FI} FI}
