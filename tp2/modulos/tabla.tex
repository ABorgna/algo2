...
\algoritmo{iBuscar}{
    ...
    \If{$t.hayIndiceNat \algAnd t.indiceNat == c$}                  \comment \bigo(1)
        \If{$definido(t.indicesNat,getNat(d))$}                     \comment \bigo($log(n) * cmp(nat)$) = \bigo($log(n)$)
            ...
            \While{$haySiguiente(its)$}
                \State \icomment Solo se repite una vez si c es clave, ya que no puede haber otro con el mismo valor
                \State \icomment Se repite a lo sumo $n$ veces si c no es clave
                \State $it \gets siguiente(its)$                    \comment \bigo(1)
    ...
    \Else
        ...
        \EndWhile
    \EndIf
}{Si c es clave y c es indiceNat: 2 * \big(1) + 2 * \bigo($log(n)$) + \bigo($L$) = \bigo($L + log(n)$) \\
    \hspace*{8.25em} Si c es clave y c es indiceString: 3 * \bigo(1) + 3 * \bigo($L$) = \bigo($L$) \\
    \hspace*{8.25em} Si no: 4 * \bigo(1) + n * (\bigo(1) + 2 * \bigo($L$) + \bigo(1)) = \bigo($L * n$)
}
